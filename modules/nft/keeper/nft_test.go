...
func (suite *KeeperSuite) TestGetNFT() {
	...
	receivedNFT2, err := suite.keeper.GetNFT(suite.ctx, classID, tokenID2)
	suite.NoError(err)
...
func (suite *KeeperSuite) TestGetNFTs() {
	...
	err = suite.keeper.MintNFT(suite.ctx, classID2, tokenID2, tokenNm2, tokenURI, tokenData, address)
	suite.NoError(err)

	err = suite.keeper.MintNFT(suite.ctx, classID2, tokenID3, tokenNm3, tokenURI, tokenData, address)
	suite.NoError(err)

	err = suite.keeper.MintNFT(suite.ctx, classID, tokenID3, tokenNm3, tokenURI, tokenData, address)
	suite.NoError(err)

	nfts := suite.keeper.GetNFTs(suite.ctx, classID2)
	suite.Len(nfts, 3)
}
...
func (suite *KeeperSuite) TestAuthorize() {
	...
	_, err = suite.keeper.Authorize(suite.ctx, classID, tokenID, address2)
	suite.Error(err)

	_, err = suite.keeper.Authorize(suite.ctx, classID, tokenID, address)
	suite.NoError(err)
}
